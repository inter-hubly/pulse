...
func (s *webSocketService) SendMessageToClient(ctx context.Context, d *dto.Message) error {
	tenant := hctx.Tenant.Get(ctx)
	conn := s.connections[tenant]
	if conn == nil {
		hlog.Info("webSocketService.SendMessageToClient", fmt.Sprintf("error when message sending to client %v", d))
		return fmt.Errorf("connection for tenant %s not found", tenant)
	}
	marshal, err := json.Marshal(d)
	if err != nil {
		hlog.Info("webSocketService.SendMessageToClient", fmt.Sprintf("error when message marshal %v", err), d)
		return err
	}
	if err = conn.WriteMessage(websocket.TextMessage, marshal); err != nil {
		hlog.Info("webSocketService.SendMessageToClient", fmt.Sprintf("error when write message %v", err), d)
		return err
	}
	return nil
}
...
func (s *webSocketService) ReceiveMessageFromClient(ctx context.Context) {
	...
	for {
		...
		if err = json.Unmarshal(message, &valueObj); err != nil {
			hlog.Info("webSocketService.ReceiveMessageFromClient", fmt.Sprintf("error when message unmarshal %v", err))
		}
		if err = s.whatsAppMediator.SendMessage(ctx, tenant, &valueObj); err != nil {
			hlog.Info("webSocketService.ReceiveMessageFromClient", fmt.Sprintf("error when send message %v", err))
		}
		hlog.Info("webSocketService.ReceiveMessageFromClient", fmt.Sprintf("Message received from client: %s", string(message)))
	}
}
...
func (s *webSocketService) CreateConnection(ctx context.Context, w http.ResponseWriter, r *http.Request) error {
	...
	if err != nil {
		hlog.Error("webSocketService.CreateConnection", fmt.Sprint("Error when create connection", err))
		return err
	}
	tenant := hctx.Tenant.Get(ctx)
...
func (s *webSocketService) DeleteConnection(ctx context.Context) {
	for tenant := range s.channelError {
		hlog.Info("webSocketService.DeleteConnection", fmt.Sprint("Deleting connection", tenant))
